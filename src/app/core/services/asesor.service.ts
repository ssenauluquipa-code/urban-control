...
export class AsesorService {
  constructor(
    @Inject(ASESOR_REPOSITORY_TOKEN) private repo: IAsesorRepository,
  ) {}
  ...
  getAsesores(
    ...
    return this.repo.getAll(term, nroDocumento, isActive, tipo);
  }
  ...
  searchAsesores(term: string, tipo?: string): Observable<IAsesorOption[]> {
    return this.repo.searchAsesores(term, tipo);
  }
}
